    ...
    if params[:id_number].present?
      ...
      if @student.blank?
        ...
        if request.referer == student_logins_url
          redirect_to student_logins_path, notice: 'Signed in successful.'
        else
        ...
        if @login.save
          if request.referer == student_logins_url
            ...
            redirect_to landing_index_path, notice: 'You are now signed in. Please find a PC to start using.'
          end
        else
          render :index
        end
